...
import com.busticket.busticket_bookingsystem.dto.*;
...
public interface ReportRepo extends JpaRepository<Booking, Long> {
    ...
    @Query(value = """
                select new com.busticket.busticket_bookingsystem.dto.YearRevenueDto(year(b.paymentDateTime), sum(b.totalPayment))
                from Booking b
    ...
    @Query(value = """
            select new com.busticket.busticket_bookingsystem.dto.MonthRevenueDto(year(b.paymentDateTime), month(b.paymentDateTime), sum(b.totalPayment))
            from Booking b
    ...
    @Query(value = """
            select new com.busticket.busticket_bookingsystem.dto.WeekRevenueDto(year(b.paymentDateTime), month(b.paymentDateTime),
    ...
    @Query(value = """
            select new com.busticket.busticket_bookingsystem.dto.CoachUsageDto(b.trip.coach.coachType, count(b.trip.coach.coachType))
            from Booking b
    ...
    @Query(value = """
    select new com.busticket.busticket_bookingsystem.dto.TopRouteDto(concat(b.trip.source.name, ' - ', b.trip.destination.name), count(b.trip.id))
    from Booking b
    where date(b.bookingDateTime) between :startDate and :endDate and b.paymentStatus='PAID'
...
""")
    ...
    @Query("""
    select new com.busticket.busticket_bookingsystem.dto.WeeklyPointsReportDto(year(t.transactionDate), week(t.transactionDate),
...
""")
    ...
    @Query("""
        select new com.busticket.busticket_bookingsystem.dto.MonthlyPointsReportDto(year(t.transactionDate), month(t.transactionDate),
            sum(case when t.transactionType = 'EARN' then t.amount else 0 end),
            sum(case when t.transactionType = 'USE' then t.amount else 0 end))
        from LoyaltyTransaction t
        where t.user.username = :username and t.transactionDate between :startDate and :endDate
    ...
    @Query("""
        select new com.busticket.busticket_bookingsystem.dto.YearlyPointsReportDto(year(t.transactionDate),
            sum(case when t.transactionType = 'EARN' then t.amount else 0 end),
            sum(case when t.transactionType = 'USE' then t.amount else 0 end))
        from LoyaltyTransaction t
        where t.user.username = :username and t.transactionDate between :startDate and :endDate
